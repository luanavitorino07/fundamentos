...
def extrair_infos(bs):
    ...
    for noticia in noticias: 
        titulo = noticia.find('h4', attrs={'class':'widget-title'}).text.strip()
        subtitulo = noticia.find('p').text.strip()
        link = noticia.a['href']

        #if(link != "https://www.aladi.org/sitioaladi/language/pt/atividadeseeventos/" and )

	    #pagina = acessar_pagina(link)

	    #titulo = noticia.find('span', attrs={'id':'view:_id1:inputText1'}).text.strip().split()

        # data = noticia.find('span', attrs={'id':'view:_id1:fecha1'}).text.strip()
        # data = f'{data[0]}/{data[2]}/{data[4]}'
	    # data_nova = data_extenso(data, '%d/%b/%Y', 'pt_BR.utf8')

        
        
        # paragrafos = []
        # paragrafos = pagina[0].find('font', attrs={'size':'2'}).text.strip().split('\n')
        ...
        # var_ambiente = 'DIR_BD'
        # nome_json = 'aladi.json'
        # inserir_db(titulo = titulo, data = data_nova, paragrafos = paragrafos, link=link, var_ambiente = var_ambiente, nome_json = nome_json )
